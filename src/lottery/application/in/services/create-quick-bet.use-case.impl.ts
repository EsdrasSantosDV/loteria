...
import { CreateQuickBetCommand } from '../commands/create-quick-bet.command';
import {
  CreateQuickBetUseCase,
  CreateQuickBetError,
  CreateQuickBetOutput,
  DrawNotFoundError,
  GameDefinitionNotFoundError,
  DrawNotOpenError,
} from '../use-cases/create-quick-bet.use-case';
...
export class CreateQuickBetUseCaseImpl extends CreateQuickBetUseCase {
  constructor(
  ...
  async doExecute(
    input: CreateQuickBetCommand,
  ): Promise<Result<CreateQuickBetOutput, CreateQuickBetError>> {
    const { drawId } = input;
    ...
    const minPickCount = definition.getPickCount().min;
    ...
    const price = definition.priceForPickCount(minPickCount);
